...
const Login = () => {
  ...
  const HandleSubmit = async () => {
    setLoading(true);
    await axios
      .get(`http://localhost:3000/users?emfail=${email}`)
      .then((res) => {
        if (res.data.length > 0) {
          if (res.data[0].password === password) {
            const user = res.data[0];
            console.log(`bienvenu m.${user.name}`);
        ...
        console.log(error);
        console.log('====================================');
        if(parseInt(error.status)===parseInt(404)){
            alert(
                'email non correct'
            )
        }
  ...
  return (
    // <div className="container d-flex justify-content-around mt-5 pt-1">
    //   <form className="col-6 mt-5 bg-light p-2 px-4 rounded shadow">
    //     <h3 className="p-2">Connectez-vous à Votre Compte</h3>
    //     {/* Email input */}
    ...
    //       <p>
    //         Pas un membre?<Link to="/register"> S'inscrire</Link>
    //       </p>
    //       <p>or sign up with:</p>
      ...
      <main className="w-full flex">
            <div className="relative flex-1 hidden items-center justify-center h-screen bg-gray-900 lg:flex">
                ...
                <div
                    ...
                    style={{
                        backgroundImage: "url(https://w0.peakpx.com/wallpaper/790/477/HD-wallpaper-croissant-crispy-food-bun.jpg)",    backgroundSize: "cover" ,  width: "100%",
    height: "100%",
                    }}
                >
                ...
                <div className="w-full max-w-md space-y-8 px-4 bg-white text-gray-600 sm:px-0">
                    <div className="">
                        ...
                        <div className="mt-5 space-y-2">
                            ...
                            <p className="">Not have an account? <Link to="/register"><a  className="font-medium text-indigo-600 hover:text-indigo-500">Sign Up</a></Link></p>
                        </div>
                    </div>
